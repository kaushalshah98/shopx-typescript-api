...
//   public async verifyuser(name: string, password: String): Promise<IUser | null> {
//     const query = niql.fromString(
//       `SELECT ${CONSTANT.BUCKET_NAME} as \`user\` 
//       FROM  ${CONSTANT.BUCKET_NAME} 
//       WHERE name= '${name}' and \`password\`= '${password}' 
...
//     return new Promise(
//       (resolve: (value?: IUser | null) => void, reject: (error?: CouchbaseError) => void) => {
//         bucket.query(query, (err: CouchbaseError, row: any) => {
//           if (err) {
//             reject(err);
//           } else if (row.length <= 0) {
//             resolve(row);
//           } else {
//             let user: IUser = row.map((userdata: any) => userdata.user);
//             resolve(user[0]);
//           }
//         });
//       }
...
//     const query = niql.fromString(`SELECT ${CONSTANT.BUCKET_NAME} as users 
//     FROM  ${CONSTANT.BUCKET_NAME} 
...
//             reject(err);
//           } else {
//             const users: IUser[] = row.map((data: any) => data.users);
//             resolve(users);
//           }
//         });
//       }
...
//     );
//   }
//   public async updateuserdata(userId: string): Promise<IUser[] | null> {
//     const query = niql.fromString(`SELECT ${CONSTANT.BUCKET_NAME} as users 
//     FROM  ${CONSTANT.BUCKET_NAME} 
//     WHERE type = '${CONSTANT.USER_TYPE}'`);
...
//     );
//   }
//   public async createuser(): Promise<IUser[] | null> {
//     const query = niql.fromString(`SELECT ${CONSTANT.BUCKET_NAME} as users 
//     FROM  ${CONSTANT.BUCKET_NAME} 
//     WHERE type = '${CONSTANT.USER_TYPE}'`);
...
//     );
//   }
//   public async sendmessage(userid: string, status: boolean): Promise<any> {
//     const query = niql.fromString(`UPDATE ${CONSTANT.BUCKET_NAME}
//     USE KEYS '${userid}'
//     set status=${status}`);
